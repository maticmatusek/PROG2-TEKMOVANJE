...
public class Igra implements Cloneable {
	...
	public Igra() {
		...
		// OB USTVARITVI IGRE SE NAREDI 19X19 IGRALNO POLJE, KI NISO VEČ ALI MANJ VSE
		// INFORMACIJE
		...
		for (Tocka t : mozna_polja.values()) {
			if (t.x == 1)
				t.sosedi.replace("Levo", "Nemore");
			if (t.x == dimenzija_igre)
				t.sosedi.replace("Desno", "Nemore");
			if (t.y == 1)
				t.sosedi.replace("Gor", "Nemore");
			if (t.y == dimenzija_igre)
				t.sosedi.replace("Dol", "Nemore");
		}
	...
	// FUNKCIJA NASTAVI DIMENZIJO IGRE V PRIMERU KASNEJŠE SPREMENITVE OZIROMA
	// NASTAVITVE NOVE IGRE
	...
	public boolean odigraj(Poteza poteza) {
		...
		if (mozna_polja.get("(" + (poteza.x() + 1) + ", " + (poteza.y() + 1) + ")").zasedenost == null) {
			...
			if (x != 1) {
				...
				if (u.zasedenost != null)
					t.sosedi.put("Levo", u.zasedenost);
			}
			if (x != dimenzija_igre) {
				Tocka u = mozna_polja.get("(" + (x + 1) + ", " + y + ")");
				u.sosedi.put("Levo", barva);
				if (u.zasedenost != null)
					t.sosedi.put("Desno", u.zasedenost);
			}
			if (y != 1) {
				Tocka u = mozna_polja.get("(" + x + ", " + (y - 1) + ")");
				u.sosedi.put("Dol", barva);
				if (u.zasedenost != null)
					t.sosedi.put("Gor", u.zasedenost);
			}
			if (y != dimenzija_igre) {
				Tocka u = mozna_polja.get("(" + x + ", " + (y + 1) + ")");
				u.sosedi.put("Gor", barva);
				if (u.zasedenost != null)
					t.sosedi.put("Dol", u.zasedenost);
			}
	...
	// FUNKCIJA DEFINIRA VSE SKUPINE KAMENČKOV IN TUDI PRAZNIH PRESEČIŠČ, KI SE
	// DRŽIJO SKUPAJ
	public void preveri_skupine() {
		...
		for (Tocka t : mozna_polja.values()) {
			if (t.zasedenost == null)
				prazne_tocke.add(t);
			else if (t.zasedenost.equals("Beli"))
				bele_tocke.add(t);
			else if (t.zasedenost.equals("Crni"))
				crne_tocke.add(t);
		...
		while (bele_tocke.size() != 0 || zbiralna_mnozica.size() != 0) {
			...
			if (zbiralna_mnozica.size() == 0) {
				if (neka_mnozica.size() != 0)
					skupine_beli.add(neka_mnozica);
			...
			if (t.sosedi.get("Levo").equals("Beli")) {
				...
				if (bele_tocke.contains(u))
					zbiralna_mnozica.add(u);
		...
		while (prazne_tocke.size() != 0 || zbiralna_mnozica.size() != 0) {
			...
			if (zbiralna_mnozica.size() == 0) {
				if (neka_mnozica.size() != 0)
					skupine_prazni.add(neka_mnozica);
	...
	public Set<Poteza> prosta_polja_okoli_skupine(String barva) {
		...
		if (barva.equals("Beli"))
			skupine = skupine_beli;
		else
			skupine = skupine_crni;
	...
	public Set<Poteza> dodaj_poteze(Poteza poteza) {
		...
		int x = poteza.x() + 1;
		int y = poteza.y() + 1;
		if (x + 1 <= dimenzija_igre && x + 1 >= 1 && mozna_polja.get("(" + (x + 1) + ", " + y + ")").equals("Prazno"))
			izhod.add(new Poteza(x, y - 1));
		if (x - 1 <= dimenzija_igre && x - 1 >= 1 && mozna_polja.get("(" + (x - 1) + ", " + y + ")").equals("Prazno"))
			izhod.add(new Poteza(x - 2, y - 1));
		if (y + 1 <= dimenzija_igre && y + 1 >= 1 && mozna_polja.get("(" + x + ", " + (y + 1) + ")").equals("Prazno"))
			izhod.add(new Poteza(x - 1, y));
		if (y - 1 <= dimenzija_igre && y - 1 >= 1 && mozna_polja.get("(" + x + ", " + (y - 1) + ")").equals("Prazno"))
			izhod.add(new Poteza(x - 1, y - 2));
		return izhod;
	}
	...
	public void preveri_igro_go() {
		...
		for (HashSet<Tocka> skupina : skupine_prazni) {
			...
			if (!preverjanje.contains("Beli") && preverjanje.contains("Crni")) {
				score_crni = score_crni + skupina.size();
			} else if (!preverjanje.contains("Crni") && preverjanje.contains("Beli")) {
	...
	public void izbrisi_skupino(HashSet<Tocka> skupina) {
		for (Tocka t : skupina) {
			t.zasedenost = null;
			if (t.x + 1 <= dimenzija_igre
					&& mozna_polja.get("(" + (t.x + 1) + ", " + t.y + ")").sosedi.get("Levo") != "Nemore")
				mozna_polja.get("(" + (t.x + 1) + ", " + t.y + ")").sosedi.replace("Levo", "Prosto");
			if (t.x - 1 >= 1 && mozna_polja.get("(" + (t.x - 1) + ", " + t.y + ")").sosedi.get("Desno") != "Nemore")
				mozna_polja.get("(" + (t.x - 1) + ", " + t.y + ")").sosedi.replace("Desno", "Prosto");
			if (t.y + 1 <= dimenzija_igre
					&& mozna_polja.get("(" + t.x + ", " + (t.y + 1) + ")").sosedi.get("Gor") != "Nemore")
				mozna_polja.get("(" + t.x + ", " + (t.y + 1) + ")").sosedi.replace("Gor", "Prosto");
			if (t.y - 1 >= 1 && mozna_polja.get("(" + t.x + ", " + (t.y - 1) + ")").sosedi.get("Dol") != "Nemore")
				mozna_polja.get("(" + t.x + ", " + (t.y - 1) + ")").sosedi.replace("Dol", "Prosto");
		}
	}
